...
const test = base.extend<TestFixtures>({
  electronApp: [
    async ({}, use) => {
       ...
       */
      let executablePattern = "dist/*/mcplens{,.*}";
      if (platform === "darwin") {
        executablePattern += "/Contents/*/mcplens";
      }
...
test.describe("Main window web content", async () => {
  test("The main window has the correct heading", async ({ page }) => {
    const heading = page.getByRole("heading", { name: "🔎 MCP Lens" });
